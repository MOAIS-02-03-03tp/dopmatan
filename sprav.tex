...
\begin{enumerate}
  ...
  \underline{Утверждение: } Для того чтобы векторное поле было соленоидальным $\Leftrightarrow \boxed{\overline{div}\overline{A}=o}$\\
  \underline{Доказательство:}
  \begin{adjustwidth}{1.5em}{1.5em}
    \circled{$\Rightarrow$} Поле соленоидальное, тогда по определению существует $\overline{B}:A=\overline{rot}\overline{B}$. 
    Вычислим $div\overline{A}=div(\overline{rot}\overline{B})=o$\\
    \circled{$\Leftarrow$} Пусть $div \overline{A}=o.$ Надо показать, что существует $\overline{B}:A=\overline{rot}\overline{B}$
  \end{adjustwidth}
\end{enumerate}
\end{document}